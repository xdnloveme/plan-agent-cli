...
export class ModelFactory {
  ...
  private static async createOpenAIModel(
    ...
    const openaiModule = await this.loadProvider('@ai-sdk/openai');
  ...
  private static async createAnthropicModel(
    ...
    const anthropicModule = await this.loadProvider('@ai-sdk/anthropic');
    const { createAnthropic } = anthropicModule as {
  ...
  private static async createGoogleModel(
    ...
    const googleModule = await this.loadProvider('@ai-sdk/google');
    const { createGoogleGenerativeAI } = googleModule as {
  ...
  private static async createCustomModel(
    ...
    try {
      const compatModule = await this.loadProvider('@ai-sdk/openai-compatible');
      const { createOpenAICompatible } = compatModule as {
